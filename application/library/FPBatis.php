...
class FPBatis {
	...
	function findMapElement($namespace, $tagName, $id) {
		$ext = APP_PATH."/application/mapper/".$namespace.'.xml';
		$tempDoc = new DomDocument();
		$tempDoc->load($ext);
		$node = $tempDoc->getElementsByTagName('sqlMap')->item(0);
	...
	public function select($params=null, $debug=false,$elm){
		$stmt = $elm->nodeValue;
		...
		return $stmt = str_replace("\r\n"," ",$stmt);
	}
	public function selectOne($id, $params=null, $debug=false){
		$ids = explode(".", $id);
		if ($elm = $this->findMapElement($ids[0], 'select', $ids[1])) {
			$stmt = $this->select($params, $debug,$elm);
			if ($debug || $this->debug)
