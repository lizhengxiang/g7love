<?php
	...
	class FPBatis {
		...
		function createConnection() {
			...
			$this->conn =new PDO($properties['dns'], $properties['username'], $properties['password'],array(
				PDO::ATTR_PERSISTENT => true
			));
			/*$data = $this->conn->prepare("SELECT * FROM registered1 WHERE id = 13");
			$data->execute();
			var_dump($data->fetchAll());exit();
			foreach( as $row) {
				print_r($row);exit();
			}
			var_dump($this->conn->query(" SELECT * FROM registered1 WHERE id = 13"));exit;
		*/
		}
		 ...
		 */
		function selectList($id, $params=null, $debug=false) {
			$ids = explode(".", $id);
